...
Wire-протокол на работающем стеке — везде OVL1. Два бинаря, отличающиеся
только в non-wire-format коде (внутренности routing'а, форматирование
логов, дефолты) — полностью interoperable; никакого скоординированного
fleet-wide апгрейда не требуется. Когда меняется wire-формат (следующая
major-версия), следуйте version-skew матрице в [WIRE_PROTOCOL.md](WIRE_PROTOCOL.md).

## Настройка seed-узла

Seed-узел — это долгоживущий публичный узел, чей `(public_key, transport, nonce)`
жёстко зашит в `node/bootstrap/seeds.rs::BUILTIN_SEEDS`. Свежие узлы без
`[[peers]]` и без `bootstrap_peers` в конфигурации используют их для входа
в сеть.
...
- Держите **минимум 3** seed'а в **разных сетевых провайдерах и
  географических регионах** — потеря одного seed'а не должна ломать bootstrap сети.
- Храните seed identities в offline cold storage (ноутбук + paper backup).
  Восстановление потерянного seed'а — fleet-wide rebuild, не runtime concern.
- Ротируйте по одному seed'у за раз; никогда одновременно.
...
Начиная с Этапа 6, демон пытается `mlock(2)`-нуть каждую аллокацию
секретного ключа — session AEAD keys, промежуточный session_kdf OKM и
(через follow-up слайсы) identity_sk / master_seed / peer_mlkem cache.
Это закрывает вектор **swap-to-disk leak**: атакующий с физическим
доступом к диску мог бы иначе восстановить ключи из swap'а спустя
минуты-дни после закрытия сессии.
...
## Гайд по дефолтному тюнингу

Дефолты ориентированы на средний Core-узел (≥4 GB RAM, ≥100 Mbps). Для
**constrained seeds** (2 GB RAM, shared VM) или **hardened public nodes**
несколько значений заслуживают явного override. **Сильно рекомендуется**
pre-deployment ревью этого раздела — неправильный дефолт на публичной
инфре — это разница между стабильным узлом и OOM-loop'ом.
...
Worst-case патологичен (очередь каждой сессии полна, каждый DHT-слот
занят). Реалистичный steady-state на публичном seed'е с ~5 000 активных
сессий и частичным DHT fill'ом — **~1–2 GB**, но придётся ограничить
`max_concurrent` и `max_store_entries`, чтобы добраться до этого из
дефолтов.

### Рекомендуемые override'ы по deployment-профилю
