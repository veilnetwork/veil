...
| `session` | Session limits | `max_concurrent` (1000), `tx_queue_depth` (4096), `idle_timeout_secs` (90) |
...
**подписанный бандл**, который на `.onion`-пути требуется **безусловно** — как
теперь и на любом другом: сырой JSON не принимает ни одна настройка. (URL вида
...
`allow_unpinned_signed_bootstrap = true`, понизить
...
Для подписи нужен **офлайновый** ключ, который лежит не там же, где config
демона. Сначала выпустите его, потом подписывайте им:

```sh
veil-cli config signer-key /etc/veil/config-signer.key
# → пишет keypair с правами 0600. По возможности держите его вне хоста:
#   перезапись осиротит все config'и, подписанные прежним ключом, — поэтому
#   для замены требуется --force.
...
Ключом `[identity]` из самого config'а подписывать **нельзя**, а эта страница
утверждала обратное — команда в таком виде перестала работать, когда
`--signer-key` стал обязательным (аудит R2-V-03). Подпись файла ключом,
лежащим в этом же файле, ничего не доказывает: кто может переписать файл,
тот может и переподписать его, и подпись подтверждает лишь то, что файл
...
щедро (≥ 4 MiB) на таких сетях. По умолчанию — `Some(1 MiB)` (audit
cycle-9): щедрый потолок, защищающий свежий узел из коробки; задайте
`None` явно, чтобы полностью отключить per-origin cap — тогда применяется
только глобальный лимит `max_store_bytes` (если выставлен).
...
|-----------|---------------|-------------|----------------|
| Session tx queues | `tx_queue_depth × avg_frame_size` ≈ 1024 × 1 KiB = 1 MiB | `max_concurrent = 1000` | **~1 GiB** |
...
```toml
[session]
max_concurrent = 8_192           # поднято заметно выше дефолта 1000 под этот RAM-бюджет
max_per_ip = 512
...
|-------|---------|-----------|-----------|
| `session.max_concurrent` | 1000 | Dedicated infra (>8 GB RAM) | Маленький seed / leaf (<4 GB) |
...
| `session.tx_queue_depth` | 4096 | Bulk-transfer воркллоады | High-concurrency RAM-lean |
| `dht.max_store_entries` | 25 000 | Dedicated DHT-инфра | **всегда для маленьких seed'ов** (→100k) |
...
| `capacity.max_inbound_bandwidth_kbps` | 10 000 000 | Enterprise-инфра | Residential / metered |
| `abuse.pow_min_difficulty` | 16 | Под атакой / ужесточение | Dev / LAN |
| `abuse.rate_limit_fps` | 200 000 | High-volume легитимные peer'ы | — |
