...
- **Асинхронную / офлайн-доставку.** У veil нет встроенного почтового
  ящика (mailbox). Если вашему мессенджеру нужна надёжная асинхронная
  доставка (сообщения, которые ждут офлайнового получателя и переживают
  перезапуск), вы строите её сами: либо отдельным крейтом
  (`veil-mailbox`, ещё не реализован), либо поверх готовых примитивов —
  `DHT.store` с TTL (запись, которую сеть удаляет по истечении времени
  жизни), самосинхронизация между вашими узлами или внешний
  ретранслятор.
...
> **Асинхронная / офлайн-доставка — вне зоны ответственности сетевого
> уровня.** У veil нет встроенного почтового ящика. Если вашему
> мессенджеру нужно хранить-и-пересылать сообщения офлайновым
> получателям, стройте это сверху. Два варианта: отдельный крейт
> `veil-mailbox` (ещё предстоит спроектировать) либо готовые примитивы —
> `DHT.store` с TTL на известном шарде (фиксированном срезе адресного
> пространства), либо выберите один онлайн-ретранслятор на каждого
> получателя и реплицируйте на него.
...
> `watcher.rs` (обнаружение аномалий), `tier_b.rs`, `mailbox/*`. Ни
> одного из них нет в текущем сетевом слое. Асинхронная доставка и отзыв
> ключей могут вернуться позже, но как **отдельные** крейты поверх
> сетевого слоя — пока не реализованы. Разделы 4-7 ниже могут
> по-прежнему ссылаться на эти API. В случае сомнений верна шпаргалка
> выше.
...
После первого удачного запроса резолвер кеширует
...
1. Отзовите новый `identity_key` с мастер-устройства.
2. Запустите наблюдатель аномалий (anomaly watcher).
3. Подумайте о полной ротации ключей — исходите из худшего и начните с
   чистого листа.
...
☐ Кеш отзывов сохраняется на диск в
...
☐ Наблюдатель аномалий запускается при каждом старте и выводит
...
☐ Обновление свежести запланировано не реже чем каждые 25 дней
(`veil-cli identity refresh-freshness` или своя автоматизация).
