...
Настройки живут в секции верхнего уровня `[hot_standby]`:

    [hot_standby]
    enabled                          = false  # opt-in; per-peer влияние на
                                              # приватность нулевое, но включение
                                              # per-peer warm probe удваивает
                                              # количество сокетов
    handoff_timeout_secs             = 5      # сколько инициатор ждёт
                                              # HandoffAck, прежде чем отменить
                                              # подмену; должно превышать RTT
                                              # основного канала
    max_swaps_per_minute             = 4      # потолок подавления дребезга
                                              # на сессию (скользящее окно 60 с)
    auto_trigger_after_write_errors  = 3      # подряд идущих ошибок записи на
                                              # основном канале до авто-запуска
                                              # передачи; 0 отключает авто-запуск
                                              # (только ручной `node swap-transport`)

Выбор alt-транспорта больше не задаётся через `alt_scheme_order`: контроллер
автоматически заполняет `alt_uri_for` из объявленных соседом URI `[[listen]]`
через capability-TLV стадии (c.3).
