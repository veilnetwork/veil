...
Поля минорной версии больше нет. `ovl1_minor` был одним из шести полей возможностей, которые всегда объявлялись и никогда не читались; single-version cleanup их вырезал, ужав `CapabilitiesPayload` с 12 байт до 2 (и до 3, когда добавился `discovery_mode`). Вместе с ними ушли и version-gate'ы, которые они питали — см. `proto/budget.rs` и раздел CapabilitiesPayload в [WIRE_PROTOCOL.md](WIRE_PROTOCOL.md).
