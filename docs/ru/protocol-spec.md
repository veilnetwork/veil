...
### 6.1 DeliveryEnvelope (197-байтный header + payload)

```text
[0..49]    recipient          Recipient (node_id[32] + tag[1] + instance_id[16])
[49..81]   sender_node_id     [u8; 32]
[81..113]  src_app_id         [u8; 32]
[113..145] app_id             [u8; 32]   (app_id получателя)
[145..149] endpoint_id        u32 BE
[149..181] content_id         [u8; 32]   (BLAKE3 payload)
[181..189] created_at         u64 BE     (Unix-время создания)
[189..193] ttl_secs           u32 BE
[193..197] payload_len        u32 BE
[197..]    payload            bytes
```

Получатель — это `Recipient` фиксированной длины 49 байт (`encode_fixed_into`): 32-байтный `node_id`, 1-байтный `InstanceTag` (0=Any, 1=All, 2=Specific) и 16-байтный `instance_id` (дополнен нулями для Any/All).
...
[32]     status      u8  (см. легенду ниже)
[33..65] mac         [u8; 32]   (C-09 аутентифицированный ACK; BLAKE3 keyed-MAC от
                                  content_id под veil_e2e::derive_ack_key)
```

WIRE_SIZE = 65 байт. Коды статуса:
- `0` = ACCEPTED — шлюз принял конверт в конвейер доставки
- `1` = DELIVERED — конечный получатель доставил payload на свой прикладной уровень
- `2` = QUEUED — почтовый ящик надёжно сохранил конверт (кворум реплик подтвердил)
- `3` = NOT_FOUND — нет записи в почтовом ящике для этого content_id
- `4` = REJECTED — доставка явно отклонена (квота, политика, авторизация)
- `5` = EXPIRED — срок конверта истёк до того, как он мог быть доставлен
- `6` = FETCHED — почтовый ящик отправил конверт получателю (извлечён из хранилища)
- `7` = APP_ACKED — приложение получателя явно подтвердило через IPC
