...
Извлечение **завершено**. Сейчас в workspace **57 участников**: 55 крейтов
под `crates/`, плюс `veilcore` и `veilclient` верхнего уровня. `veilcore`
остался тонким остатком — sim, интеграционная обвязка и несколько шимов
`node/*`, — а извлечённый рантайм узла живёт в **`veil-node-runtime`**.
Число тестов по крейтам меняется быстро: живые значения смотрите в
`cargo nextest list`.

Полный перечень крейтов по категориям — в английской версии этого документа,
[`docs/en/CRATE_ARCHITECTURE.md`](../en/CRATE_ARCHITECTURE.md); здесь ниже
сохранён СНИМОК, сделанный по ходу миграции, вместе с числами тестов на тот
момент. Он намеренно не дополняется крейтами, появившимися позже.
