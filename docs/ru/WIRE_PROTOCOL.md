# Формат на проводе OVL1

> Побайтовый справочник для тех, кто реализует протокол. Он описывает, что идёт по
> проводу, поле за полем. Полное описание системы — в
> [ARCHITECTURE_FULL.md](ARCHITECTURE_FULL.md). Что означает каждая полезная
> нагрузка — в [protocol-spec.md](protocol-spec.md).

## Заголовок кадра (24 байта)

Каждое сообщение на проводе — это *кадр* (frame): фиксированный 24-байтный
заголовок и за ним необязательное тело. Заголовок ниже одинаков у всех кадров;
многобайтные поля записаны в порядке от старшего байта (big-endian, BE).
...
### Биты приоритета (flags[1:0])

Младшие два бита `flags` задают класс приоритета кадра. Планировщик обслуживает
четыре класса по взвешенному круговому обходу (weighted round-robin, WRR), так что
под нагрузкой больший вес получает больше очередей.
...
Все остальные биты `flags` зарезервированы. Отправитель ОБЯЗАН их обнулять;
получатель ОБЯЗАН игнорировать любой бит, который он не распознаёт.

## Семейства кадров

*Семейство* (family) собирает родственные типы сообщений под одним байтом `family`,
а поле `msg_type` выбирает конкретное сообщение внутри семейства. Числа в скобках
ниже — это значения `msg_type`. Источник истины — файл
...
| 2 | Discovery | FindValue(1), Store(2), Delete(3), AnnounceAttachment(4), GetAttachment(5), GetAppEndpoint(7), FindValueResponse(9), FindNodeV2(10), FindNodeV2Response(11), ResolveTransport(12), ResolveTransportResponse(13), AnnounceTransport(14) — слоты 0/6/8 удалены (не выделены) |
| 3 | Delivery | Forward(3), DeliveryStatus(4), ChunkManifest(7), Chunk(8), Transit(0x10), RecursiveRelay(0x11), RelayPath(0x12) — mailbox-слоты 0/1/2/5/6 удалены (не выделены) |
| 4 | App | AppOpen(0), AppData(1), AppClose(2), AppSend(3), AppReceipt(4), AppWindowUpdate(5), AppRtData(6) |
| 5 | Mesh | Forward(0), Beacon(1), Ack(2) |
| 6 | LocalApp | 72 типа IPC-сообщений (AppHello=0 … SetWakeHmacEnvelopeOk=71); полный список — в [`family.rs`](../../crates/veil-proto/src/family.rs) |
...
Кадр с неизвестным `family` просто игнорируется — это и даёт совместимость с
будущими версиями: старые узлы пропускают непонятные им сообщения, а не ломаются.
Неизвестный `msg_type` внутри уже знакомого семейства игнорируется так же.

## Последовательность рукопожатия

Прежде чем пойдёт настоящий трафик, обе стороны выполняют *рукопожатие* (handshake)
— строго заданный обмен, который подтверждает личность и согласует общий ключ.
Каждый шаг ниже клиент отправляет, а сервер повторяет в ответ, по порядку:
...
Начиная с `SessionConfirm`, тело каждого кадра шифруется алгоритмом
ChaCha20-Poly1305. 24-байтный заголовок остаётся открытым текстом, но подаётся
шифру как дополнительные аутентифицируемые данные (AAD): он не скрыт, однако любая
его подмена ломает расшифровку.

## Ключевые полезные нагрузки

*Полезная нагрузка* (payload) — это тело кадра, байты после 24-байтного заголовка.
Каждая раскладка ниже задаёт диапазоны байтов в виде `[начало..конец]`,
полуоткрытые (конечный индекс не входит), в том же порядке от старшего байта, что и
заголовок.

### HelloPayload (34 байта)
```
...
### CapabilitiesPayload (3 байта, wire v3)
```
...
> `max_frame_size`, `max_streams` и `ovl1_minor`, а вместе с ними и флаги
> `CAN_MAILBOX/CAN_GATEWAY_LOCAL_MESH/CAN_PARTICIPATE_DHT/`
> `CAN_ACCEPT_APP_STREAMS/CAN_STORE/SUPPORTS_TRANSIT`. Ради совместимости декодер
> всё же принимает 2-байтную форму (`roles + flags`); в этом случае он
> подставляет `discovery_mode = Public`.

### DeletePayload (variable)
Аутентифицированный запрос на удаление значения из DHT. Он несёт подпись, поэтому
убрать запись может только её законный владелец. Поддерживаются все алгоритмы
подписи личности — Ed25519, Falcon-512 и гибриды Ed25519+Falcon-512/1024 — через
...
Сервер принимает удаление, только если проходят все три проверки:
1. алгоритм входит в число разрешённых здесь — `algo ∈ {0, 2}`;
2. подпись верна — `crypto::verify_message(algo, public_key, key_bytes, signature)` возвращает OK;
3. ключ принадлежит подписавшему — `BLAKE3(public_key) == key`, так что удалить запись может только узел, чей `node_id == key`.

### RecursiveRelayPayload
```
[0..32]   dst_node_id     final destination
[32..64]  originator_id   relay initiator (for reverse-path cache)
...
### E2eEnvelope (с маркер-байтом 0xE2 или 0xE3 в начале DeliveryEnvelope.payload)
```
[0]         version        u8 = 1
...
## Константы — быстрый справочник

Несколько пределов, которые удобно держать под рукой. Источник истины —
[`budget.rs`](../../crates/veil-proto/src/budget.rs); таблица ниже лишь повторяет
его для удобства.
