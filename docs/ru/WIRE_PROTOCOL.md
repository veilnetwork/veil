...
> Field-level reference для разработчиков, имплементирующих протокол.
> Полное системное описание — в
> [ARCHITECTURE_FULL.md](ARCHITECTURE_FULL.md); семантика per-payload —
> в [protocol-spec.md](protocol-spec.md).

## Frame header (24 байта)
...
Все остальные биты `flags` зарезервированы; sender ОБЯЗАН их обнулять,
receiver ОБЯЗАН игнорировать неизвестные биты.
...
| 10 | RelayChain | Hop(0) — onion-encrypted relay chain; RegisterRendezvous(1), UnregisterRendezvous(2), ForwardIntroduce(3) — plain control-payload'ы поверх установленной сессии (без onion-шифрования) |
| 11 | PeerExchange | Walk(0), Challenge(1), Response(2), Result(3) |

Неизвестный `family` → frame игнорируется (forward-compatible). Неизвестный
`msg_type` внутри известной family — то же самое.

## Последовательность handshake'а
...
24-байтный header остаётся в plaintext и используется как AAD.
...
```
> Wire v3 убрал старую 12-байтную форму: `transports_supported`, `max_frame_size`,
> `max_streams`, `ovl1_minor` и флаги `CAN_MAILBOX/CAN_GATEWAY_LOCAL_MESH/CAN_PARTICIPATE_DHT/`
> `CAN_ACCEPT_APP_STREAMS/CAN_STORE/SUPPORTS_TRANSIT` удалены. Декодер также принимает
> 2-байтную форму (`roles + flags`), подставляя `discovery_mode = Public`.

### DeletePayload (variable)
Authenticated DHT delete. Поддерживает все алгоритмы подписи идентичности (Ed25519, Falcon-512 и гибриды Ed25519+Falcon-512/1024) через `SignatureAlgorithm::from_wire_byte`.
...
Валидация на server'е:
1. `algo ∈ {0, 2}`;
2. `crypto::verify_message(algo, public_key, key_bytes, signature)` = OK;
3. `BLAKE3(public_key) == key` — удалить может только тот узел, чей
   `node_id == key`.
...
Авторитетный список — в [`budget.rs`](../../crates/veil-proto/src/budget.rs).
