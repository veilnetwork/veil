...
(attachment records) от имени leaf-узлов. У Core-узла она включена по умолчанию, а
отключается через `[gateway] enabled = false`. Никакого флага возможностей за
ней нет: бит `CAN_GATEWAY_LOCAL_MESH`, который здесь назывался, вырезан в
single-version cleanup вместе с пятью другими — их объявляли и никогда не
читали.
