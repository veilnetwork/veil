...
> **Note (current as of v0.2.0):** This RFC records the architecture as accepted in 2026-03. Some details have since changed in implementation — most notably the node-role model collapsed to two roles **leaf** and **core** (the separate `gateway`/`relay` roles were folded into `core` in Epic 435), and the mailbox is a single **redb** store (not memory/wal/rocksdb). See `docs/en/ARCHITECTURE.md` / `identity-model.md` / `developer-guide.md` for the current design.
...
- **`core`**: full Kademlia participant, stores and replicates records; also provides gateway, relay, and exit-proxy functions (gated by config flags).
- **`leaf`**: never owns DHT records.
...
[proxy.exit]
enabled = true   # only on Core nodes with can_exit_proxy
```

Only `Core` nodes with `exit.enabled = true` accept exit proxy connections.
...
Backend: a single embedded `redb` key-value store on disk. (This RFC predates the redb migration; the earlier `memory`/`wal`/`rocksdb` backend selection no longer exists.)
