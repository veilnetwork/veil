...
* QUIC + Noise IK + post-quantum hybrid key exchange (ML-KEM-768
  + X25519) for inter-veil sessions.

### Do you provide a way for users to request that their data be deleted?

**Answer:** Yes. Users can:
* Delete the app, which wipes the on-device identity and message
  history.
* Use `veil-cli identity delete` (or the in-app equivalent) to
  cryptographically retire their sovereign identity (this publishes a
  RevocationCert to the DHT so old keys cannot be impersonated).
...
Delete identity (publishes a RevocationCert to the DHT and retires
the sovereign key cryptographically; restoration is impossible
without the 24-word BIP-39 phrase the user explicitly chose to back
up).
