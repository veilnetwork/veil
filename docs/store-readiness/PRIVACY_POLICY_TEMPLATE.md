...
* Identity signatures: Ed25519 + Falcon-512 (post-quantum-resistant
  authentication even if quantum computers break Ed25519).
...
* In transit: relays drop frames after delivery (typical retention
  of cached encrypted blobs: 7 days (default; configurable) per-deployment).
...
* **Deletion:** uninstall the app, OR in-app: Settings → Delete
  identity (a local wipe of your identity and private keys on the
  device; no revocation is published to the network, and any
  short-lived delegated subkeys expire on their own).
