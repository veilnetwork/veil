...
> **Status (2026-06): ✅ v1 SHIPPED** (Ed25519 + single-cell + one-way +
> production-wired). Chosen direction: production onion-routed delivery (relay
> anonymity) **with the recipient authenticating the sender** — the
> "Signal-over-Tor" model: relays learn neither sender↔recipient nor content;
> the recipient cryptographically verifies WHO sent the message but never learns
> WHERE (the sender's network location). Code: `crates/veil-identity/src/auth_deliver.rs`;
> domain separator `AUTH_APP_DELIVER_DOMAIN = b"veil-auth-onion-deliver:v1\0"`
> (veil-proto); the `anonymous_authenticated` IPC send flag is wired through
> `crates/veil-ipc/src/handlers/send.rs` + `crates/veil-proto/src/ipc.rs` with
> round-trip tests. **Deferred to v2** (unchanged, see the §0 banner above):
> hybrid-Falcon signature, replies / bidirectional, stateful circuits (482.7),
> Double Ratchet.
...
## 6. Production wiring (✅ shipped — was sim-only at design time)
