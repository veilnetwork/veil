...
Status: **SHIPPED (2026-06, post-Epic-482).** The location-anonymous-service /
return-path subset is in `main` — see TASKS.md §"Onion-registration /
location-anonymous service" and the §3.B "As-built (b4a)" note below. Shipped:
registration over an onion circuit (R never learns the service location),
stateful return circuits (the 482.7 return-path subset), cookie-keyed forward of
introduces down the circuit, first-registration-wins cookie registry, and
maintenance-tick circuit rebuild-on-TTL. Code lives in `crates/veil-anonymity`
(`circuit_register` / `circuit_table` / `circuit_data` / `circuit_setup` /
`circuit_wire`); prod entry is the `[anonymity].onion_service` config flag + the
`RegisterOnionService` IPC verb. **Still deferred:** reusing a circuit for N
*forward* messages to the same destination — the send side still builds a fresh
onion per message (482.7 forward-circuit reuse; see that backlog row). The design
text below is preserved as the original sketch.
