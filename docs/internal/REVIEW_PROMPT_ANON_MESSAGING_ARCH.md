...
onion-routing path (`send_anonymous`) is **sim/bench-only, not IPC-wired** (see
`docs/internal/onion-path-not-prod-wired` reasoning; verify:
`grep -rn '\.send_anonymous(' --include=*.rs .` → only `veilcore/src/sim/`).

The team chose a new direction: **"Signal-over-Tor for veil"** — Tor-style
