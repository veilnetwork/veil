...
The knobs live under the top-level `[hot_standby]` section:

    [hot_standby]
    enabled                          = false  # opt-in; privacy impact is nil on a
                                              # per-peer basis but enabling per-peer
                                              # warm probes doubles socket count
    handoff_timeout_secs             = 5      # how long the initiator waits for
                                              # HandoffAck before aborting the swap;
                                              # must exceed the primary's RTT
    max_swaps_per_minute             = 4      # per-session flap-damping ceiling
                                              # (rolling 60-second window)
    auto_trigger_after_write_errors  = 3      # consecutive primary write errors
                                              # before auto-triggering a handoff;
                                              # 0 disables the auto-trigger (manual
                                              # `node swap-transport` only)

Alt-transport selection is no longer a configured `alt_scheme_order`: the
controller auto-populates `alt_uri_for` from peer-advertised `[[listen]]` URIs
via the stage-(c.3) capability TLV.
