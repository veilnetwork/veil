...
- A random 16-byte name, the **`instance_id`**. It's written to
  ...
  This locally-persisted random id is what peers target with
  `Recipient::Specific(instance_id)`. (Separately, a verified
  identity result also exposes an `active_instance_id` derived
  deterministically as `device_id[..16]` — a backward-compat
  stand-in for older code paths, distinct from this random id;
  see [identity-model.md](identity-model.md).)
- Its own Ed25519 signing key, the **`identity_sk`**. Each one is
  certified by the master key on its own, with its own short
  `valid_until_unix` window. So if one device's `identity_sk` is
  stolen, only *that* subkey is affected — you stop re-issuing it
  and it expires; the rest keep working.
- Its own ML-KEM-768 **encryption keypair**, certified under that
  same `identity_sk`.
- A slot in the shared **`InstanceRegistry`** — the list every
...
> ⚠️ **There is no `identity revoke` command — and no revocation
> mechanism at all.** Removing a device means you stop endorsing
> its subkey and let it expire: from a device that holds the master
> seed, rotate to a fresh subkey and re-publish the
> `IdentityDocument`. The removed device's delegation then ages out
> on its own once its `valid_until_unix` window lapses. The
...
Whoever holds the master does this. From the master device, drop
that device's subkey from `IdentityDocument.identity_keys`, re-sign
the document with an active subkey, and re-publish it. There is no
`revoked_keys` set and no `document_version` counter — the removed
subkey is simply no longer present (and, even if it lingered, its
own `valid_until_unix` would soon expire, because the master stops
re-issuing it). An identity holds at most `MAX_IDENTITY_KEYS = 8`
live subkeys, so the document stays small. Once a peer fetches the
fresher document, the dropped subkey is gone from the key list; any
delegation it once held ages out within its 7-day window. To remove
the device from routing, also re-publish the `InstanceRegistry`
(with a higher `reg_version`) minus its `InstanceEntry` — peers
reject a registry whose `reg_version` goes backwards, so the
updated list wins.
...
reputation for long: that subkey carries a short `valid_until_unix`
window, so it drops off the network once the master stops
re-issuing it, capping how long the damage can pile up.
...
**A:** Sixteen, the `InstanceRegistry` cap (`MAX_INSTANCES = 16`). For bigger fleets,
split the load across several identities.

**Q:** What happens if I remove my laptop but my phone is offline
for a week?

**A:** The phone keeps accepting messages addressed to it
specifically. When it reconnects, it pulls the latest
`IdentityDocument`, sees that its sibling's subkey is gone (or has
expired), and shows the operator a DeviceLinkedEvent-style alert.
Your name, your reputation, and the phone's own identity_sk are all
untouched.

**Q:** What if my phone is compromised while my laptop is offline?

**A:** From the laptop, once it's back online, rotate and drop the
phone's subkey, then re-publish the `IdentityDocument`. Until the
fresher document reaches a given peer — by DHT republish, or by
direct push when sessions reconnect — that peer may still accept
frames signed by the phone's key, but only until that key's own
`valid_until_unix` (7 days by default) lapses. Worst case, the DHT
path takes hours. With gossip, nearby peers pick it up in seconds.
