...
[81..113]  src_app_id
[113..145] app_id          (of the recipient)
...
The recipient is a fixed-49-byte `Recipient` (`encode_fixed_into`): `node_id[32]` + 1-byte `InstanceTag` (0=Any, 1=All, 2=Specific) + 16-byte `instance_id` (zero-padded for Any/All).
...
```
It uses saturating arithmetic ([`nat/discovery.rs` ICE priority](../../crates/veil-nat/src/discovery.rs)).
