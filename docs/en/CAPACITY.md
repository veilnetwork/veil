...
| **Sessions** | up to `max_concurrent` per node (default 1000, tunable higher) | ~20 | ~200 | ~1K | ~5K | ~10K |
...
|-----------|--------|--------------|-----------|
| Sessions | highest | 1000 (default `max_concurrent`, tunable higher) × 256 × 256B ≈ 64MB (capped by eviction) | Budget-managed |
...
| Pubkey Cache | low | 1000 × 80B ≈ 80KB (scales with `max_concurrent`) | 10MB |
| Vivaldi | lowest (evicted first) | 1000 × 48B ≈ 48KB (scales with `max_concurrent`) | 5MB |
