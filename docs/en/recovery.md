...
signing key for a fresh one: the master certifies a new short-lived
`identity_sk` for this device, and the old one simply stops being re-issued
and expires on its own within its `valid_until_unix` window (7 days by
default). There is no revocation list — old keys age out, they aren't
cancelled. Your name, contacts, reputation, and linked devices all carry
over untouched.
...
Your other linked devices pick up the freshly re-published `IdentityDocument`
on their own. Over the DHT — the network's shared address book — that takes
about 6 hours. Devices
...
subkey (a per-device signing key) and re-publishes the document. Nothing old
is revoked. The previous device's `identity_sk` stays valid until its own
`valid_until_unix` window lapses, because it was never compromised — the
device was only lost, not stolen. So if you later *find* that device, its
`identity_sk` still works for as long as that delegation remains within its
validity window (and renews itself on the maintenance tick if the device is
running).
...
# Master-certifies a fresh short-lived identity_sk, points the
# active sig_key_idx at it, and persists the updated signed
# IdentityDocument.  The running daemon publishes the update to
# the DHT and pushes it to currently-connected peers.  The old
# subkey is simply no longer re-issued, so it ages out within its
# valid_until_unix window (7 days by default).
```

> There is no standalone `identity revoke` subcommand, and no revocation
> mechanism at all. `identity rotate` mints a fresh subkey and stops renewing
> the old one; the old delegation expires on its own.

There is no live revocation channel and no revoked-keys list — Veil does not
broadcast "cancel this key now." Instead the old delegation carries a short
`valid_until_unix` (7 days by default), and once that timestamp slips into the
past every verifier rejects it (a `KeyExpired` failure). A fresh rotation just
means you stop re-issuing the old key, and it dies on schedule.
...
# signed IdentityDocument): the identity_keys list with each key's
# valid_until_unix, the active sig_key_idx, and the document's own
# valid_until_unix freshness window.
```

Compare the printed `identity_keys` and `sig_key_idx` against what you expect.
Suppose an attacker managed to add a subkey (an unexpected pairing) or point
`sig_key_idx` at a key you don't recognise. A fresh `identity rotate` on the
device that holds your master key takes back control. It re-publishes a
master-signed document with a current `valid_until_unix`, which is the freshest
copy in the DHT, so peers fetching your identity converge on it. Only
`master_sk` can certify subkeys, and the attacker doesn't have it — more on
that below.

**What an attacker with a stolen `identity_sk` can actually do, until the
stolen delegation's `valid_until_unix` lapses:**

- Sign `IdentityDocument` updates — but only with the document's own
  `valid_until_unix` window (at most 30 days), and they cannot certify any new
  subkey, because that needs `master_sk`. Once the stolen subkey's per-key
  `valid_until_unix` (7 days by default) passes, verifiers reject anything it
  signed.
- Sign `NameClaim` updates. Your later rotate-and-republish supersedes theirs.
- Sign `InstanceRegistry` updates. A peer rejects any registry whose
  `reg_version` goes backwards, so your next signed registry (with a higher
  `reg_version`) wins.
...
- Extend a delegation past its `valid_until_unix` — re-issuance needs
  `master_sk`, so the stolen key ages out no matter what.
...
break-in on one device only ages out that one device's key. Your other devices
keep working.
...
| Per-device identity_sk | No (old one expires; fresh one certified) | No (fresh one generated) |
...
- A verifier rejects any `IdentityDocument` once `now > valid_until_unix`. That
  window is capped at 30 days (`MAX_FRESHNESS_WINDOW_SECS`), and the owner must
  re-publish before it expires. So if you go fully offline for more than 30
  days, peers that never saw a fresh document won't connect until you come back
  online and re-publish. (Peers who *did* see a still-valid copy hold on to it.)
  This is the *only* freshness mechanism — there is no separate
  `master_freshness_sig` field.
- Every per-device delegation carries its own `valid_until_unix` (7 days by
  default). A compromised subkey ages out on its own once that timestamp
  passes; there is no revocation cache or revoked-keys list to keep in sync,
...
- A peer rejects any `InstanceRegistry` whose `reg_version` goes backwards. You
  ...
  compromised every current key, because publishing a higher `reg_version`
  later always wins.
