...
- `MAGIC = "OVL1"`
- `VERSION = 2` (was 1 until 2026-08-02: the AEAD associated data grew
  from three bytes to the whole 24-byte header, and the two are not
  interoperable — a v1 peer is refused at `decode_header` with
  `UnsupportedVersion`)
- `HEADER_SIZE = 24`
