...
for everything that follows. Veil's handshake takes five round-trips, and
...
Sender A wants to reach D, no cached route:
   ...
   N3: do I have a direct session to D?
       ...
       ...
   After ≤ 20 hops, or mailbox fallback if D is offline.
```
...
Everything above describes **OVL1 v2** (magic `0x4F564C31`, version byte
`0x02`). The protocol grows by having nodes negotiate which features
