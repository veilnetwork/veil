...
```toml
[session]
max_concurrent = 8_192           # raised well above the 1000 default for this RAM budget
max_per_ip = 512
...
|-------|---------|-----------|-----------|
| `session.max_concurrent` | 1000 | Dedicated infra (>8 GB RAM) | Small seed / leaf (<4 GB) |
...
| `session.tx_queue_depth` | 4096 | Bulk-transfer workloads | High-concurrency RAM-lean |
| `dht.max_store_entries` | 25 000 | Dedicated DHT infra | **always for small seeds** (→100k) |
...
| `capacity.max_inbound_bandwidth_kbps` | 10 000 000 | Enterprise infra | Residential / metered |
| `abuse.pow_min_difficulty` | 16 | Under attack / tighten | Dev / LAN |
| `abuse.rate_limit_fps` | 200 000 | High-volume legit peers | — |
