...
| `strict_config_validation` | `bool` | `false` | Production-hardening profile. When `true`, the daemon promotes the production-posture config advisories (a push relay without `mailbox.push.require_wake_hmac`, a mailbox without `mailbox.require_capability_token`, `dht.allow_unsigned_store`, etc.) from non-fatal warnings to **fatal** errors and refuses to start until they're addressed. Pairs naturally with `require_signed_config = true`; it only promotes existing advisories, adding no new checks |
| `allow_unpinned_signed_bootstrap` | `bool` | `false` | Accept a signed bootstrap bundle whose issuer isn't pinned via `trusted_bundle_issuer_pubkey`. Default `false` (fail closed — an attacker controlling the HTTPS origin could serve their own validly-signed bundle). Set `true` only for dev/testnet; production must provision `trusted_bundle_issuer_pubkey` instead |
...
| `max_lazy_difficulty` | `u8` | `32` | The highest difficulty the lazy miner will attempt |
...
| `core` | A full network participant: DHT (K=20), relay/forwarding, gateway (attachment records), and mailbox. Recommended PoW difficulty ≥ 24 (the `--difficulty` default is `24`; `MAX_POW_DIFFICULTY = 24` is the hard cap) |
...
| `max_concurrent` | `usize` | `1000` | Most OVL1 sessions open at once |
...
refreshes. A v4 record carries its own signed `issued_at` and is aged by that
