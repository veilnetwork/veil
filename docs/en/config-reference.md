...
| `webtunnel_response_floor_ms` | `int` (ms) or absent | unset / `0` | Optional anti-probe response-timing floor. When set, the server holds every response — the tunnel `101` upgrade and the decoy alike — until at least this many milliseconds have passed since the request was read, so a prober can't tell a real tunnel endpoint from a plain decoy by latency. Costs up to this much added handshake latency, so it's opt-in. Set it above the decoy's worst-case fetch time (for a proxy decoy, above the backend's latency), or the decoy can still overrun the floor. `0`/unset responds as soon as ready |
