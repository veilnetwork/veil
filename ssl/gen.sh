#!/usr/bin/env bash
# Generate a local self-signed dev cert + private key.  NOT for production.
#
# Hardening notes:
...
#     left behind.
#   * The .pem files are excluded from the Docker context via `.dockerignore` to
...
# umask was already restrictive (no-op) or openssl created the file
# with broader perms (some versions ignore umask for -keyout).
