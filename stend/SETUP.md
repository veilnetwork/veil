...
After cloning the repo the per-node identity files and `config.toml`
...
keys, which was a security violation if the repo were ever published
or forked).
...
- `stend/node{1..5}/config.toml` (with a populated `[Identity]` section)
- `stend/node{1..5}/veil.txt` (operator's notes — empty by default)

## Why split out

Stend is a dev-only convenience — replacing each node's identity
between runs is fine, but committing keys was a mistake.  The setup
script is idempotent (skip-if-exists) so re-running is safe.

## Removing residue from a previous clone

If you cloned before this fix, your working tree may still have the
stale committed files.  Remove them with:
...
`ssl/gen.sh` issues a 365-day self-signed cert for `example.local`.
