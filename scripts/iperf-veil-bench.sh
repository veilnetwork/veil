#!/usr/bin/env bash
# iperf-veil-bench.sh — integration bench для veil throughput.
#
# Two-part test:
#   1. baseline = iperf3 over kernel loopback (127.0.0.1)
#   2. veil  = 2 daemons + 2 ogates в network namespaces, iperf3 between
#                 the virtual ogate IPs
#
# Compares (2) к (1) и fails если veil throughput drops below а
...
#   DEVNET_DIR         working dir для devnet state (default
#                       /tmp/iperf-veil-bench)
#   KEEP               if "1", skip cleanup на exit (debug)
#
# Exit codes:
#   0 — pass
...
#   2 — setup error
#
# Requires:  sudo (TUN + netns + iptables), iperf3, veil-cli и ogate
...
# Threshold sized к catch catastrophic regressions (e.g. the Phase E27
# batching bug that dropped veil throughput from 4-5 % к 0.001 % of
# loopback), не minor variance.  Local single-host runs naturally see
...
WARMUP="${WARMUP:-3}"
# Number of veil-measurement trials; the best ratio is reported и used
# for the pass/fail decision.  Single-host benches are noisy enough that
# а single sample can spuriously dip below the threshold даже на а healthy
...
# Loopback bind для phase-1 iperf3 — keep на а high port to avoid
...
# Locate the actual mbps number из iperf3 sender-summary line. iperf3 prints
...
if ! sudo -n true 2>/dev/null; then
    err "sudo with NOPASSWD required для TUN/netns ops"
    exit 2
fi
...
log "phase 2/2: veil (2 daemons + 2 ogates через netns)"
...
# Generate fresh configs each run — avoids any leftover state из prior runs.
# PoW difficulty 24 matches production builds' runtime check; each `config
# init` runs а short search (typically 1-30 s on modern hardware).
...
for n in 0 1; do
    ...
    "$VEIL_CLI" config init "$cfg" --difficulty 24 >/dev/null 2>&1 || {
        err "config init failed для node-${n}"
        exit 2
    }
    ...
    # on а single-host bench to avoid scheduler thrashing.
...
# Wait для daemons к bind admin + ipc sockets и handshake their peer session.
for _ in {1..15}; do
...
# Wait для session establishment.
for _ in {1..10}; do
...
# Get node_ids для ogate configs.
...
# Start ogate.  `OGATE_WORKERS=1` keeps the tokio runtime small enough к
# coexist с another ogate + 2 daemons + iperf3 endpoints на one host.
...
# session-runner's TX queue.  Fresh-process cold-start у TCP slow-start
# + first-frame ChaCha20 keystream init add ~1-3 s of below-steady-state
# throughput, which would skew the headline number on а short bench.
# Discarded run; logfile к /dev/null.
if [[ "$WARMUP" -gt 0 ]]; then
...
# iperf3 over veil — repeat TRIALS times и keep the best.  Single-host
# benches naturally vary ±30 % due к kernel scheduler + iperf3 own thread
# placement; taking max(N) gives а stable indicator of the architectural
# ceiling, не the worst-case scheduler luck.
VEIL_MBPS=0
TRIAL_RESULTS=()
for i in $(seq 1 "$TRIALS"); do
    ...
    mbps=$(parse_mbps "/tmp/iperf-veil-trial-$i.log") || {
        err "trial $i failed к parse iperf3 output"
