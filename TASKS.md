...
## Onion-registration / location-anonymous service (shipped 2026-06, post-Epic-482)

A Tor-hidden-service analogue, built brick-by-brick on top of Epic 482's anonymity
layer. Not a numbered epic — emerged from the reply-channel work. All in `main`.

- **Location-anonymous service registration** (`81e7130`, b1–b7): a receiver registers
  at a rendezvous relay R **over an onion circuit** (not a direct session), so R never
  learns the service's location. Stateful return circuits (the 482.7 return-path subset
  — see that backlog row), cookie-keyed forward of introduces down the circuit, fixed-size
  cells (length-preserving BLAKE3-XOF layers), circuit rebuild-on-TTL via the maintenance
  tick. Prod entry: `[anonymity].onion_service` config flag + IPC `RegisterOnionService`.
- **Reply channel** (r5–r7 + 1c): bidirectional authenticated anonymous messaging via
  one-time reply blocks; the reply cookie is also registered over a circuit so neither
  side leaks its location. Non-consuming-until-TTL (retry-friendly, at-least-once).
- **Tor-v3 blinded descriptors** (`3b500f2` + `c2729a5`): a service publishes an UNLINKABLE
  per-period descriptor — keyed in the DHT under `H(blinded_public(identity, period))`,
  encrypted under an identity+period key, signed by the per-period blinded key
  (`veil-crypto::key_blinding`, `veil-anonymity::blinded_descriptor`). A DHT enumerator sees
  only a rotating key + opaque ciphertext; a client that knows the service identity resolves
  + decrypts it. Re-published each maintenance tick (it rotates per 24 h period).
  ⚠️ **CAVEAT**: uses `ed25519-dalek` hazmat; validated only for veil↔veil self-consistency
  (round-trip + e2e), NOT Tor interop and NOT independently crypto-audited — flag before any
  security audit.
- **By-identity + direct anonymous send** (`d9048b0` and predecessors): four addressing
  modes across SDK/FFI/Flutter — authenticated/anonymous × by-node_id/by-identity — plus
  `send_anonymous_direct` (the last sim-only onion-send path, now prod-wired). No onion-send
  path remains sim-only.
