...
| Stateful `CircuitId` for persistent circuits | 482.7 | ⚠️ **return-path subset SHIPPED** (onion-registration epic, 2026-06; see the "Onion-registration / location-anonymous service" note in [`TASKS_ARCHIVE.md`](TASKS_ARCHIVE.md)). Stateful circuits now exist end-to-end: per-hop key install (`circuit_setup`), dual-indexed cookie-keyed relay table (`circuit_table`), first-wins cookie registry (`circuit_register`), build-once return circuits kept alive by the maintenance tick, and an **anti-replay window per circuit** (`circuit_data::ReplayWindow`). What REMAINS deferred: reusing a circuit for N **forward** messages to the same destination — the send side (`send_anonymous*` / `send_via_rendezvous*`) still builds a fresh onion per message. | Perf-driven need: interactive chat shows high re-build overhead vs message latency (now narrowed to the forward/send leg — the return leg is stateful). |
...
| Anycast first-use Sybil / peer-controlled score (EXT#4) | Audit remediation batch 2026-06-14 | `veil-anycast/src/lib.rs` reputation is post-failure only, so a first-use anycast target can be a fresh Sybil.  *Proposal:* quorum/stake-weighted first-use admission (≥k independent attesters before a target is eligible) — needs a reputation-quorum design, not a point fix.  (Closely related to Audit cycle 4 T2 anycast/relay reputation feedback loop.) | Anycast moves onto an untrusted-relay path. |
| Unauthenticated `CircuitBuilt` ACK MAC (M-3) | Audit remediation batch 2026-06-14 | `veil-anonymity/src/circuit_wire.rs` — `CircuitBuiltPayload` is `[circuit_id u32]` with no MAC; a path member could forge premature build-confirmation.  *Proposal:* bind the ACK with a per-hop MAC keyed by the circuit key. | Circuits extend across nodes outside the current trust assumption. |
| Bandwidth-mimicry implementation (EXT#2) | Audit remediation batch 2026-06-14 | Landing-pad ready + fail-closed (`transport_glue.rs` warns loudly; `cfg-validate` rejects `bandwidth_mimicry_enabled = true` without the no-op acknowledgement flag).  *Proposal:* implement the shaping profile once the operator supplies a representative pcap fixture. | The operator pcap fixture lands. |
| pqcrypto migration of remaining classical-only paths (EXT#5) | Audit remediation batch 2026-06-14 | *Proposal:* stage behind the existing PQ feature flags as upstream pure-Rust ML-KEM/ML-DSA stabilise. | A maintained pure-Rust ML-KEM/ML-DSA backend is GA. |
| Falcon / `pqcrypto-*` → `fn-dsa` migration | Dependency-audit rationale refresh 2026-08-04 | **Distinct from EXT#5 above** — that row is about extending PQ to paths that are still classical-only; this one is about replacing the *backend* of the Falcon path we already ship. Upstream PQClean is archived, so `pqcrypto-falcon` / `pqcrypto-traits` / `pqcrypto-internals` are flagged unmaintained (RUSTSEC-2026-0165 / -0162 / -0163) and carried as accepted risks in [`deny.toml`](deny.toml) + [`.cargo/audit.toml`](.cargo/audit.toml). All three are pulled in for Falcon and nothing else (there is no `pqcrypto-mlkem` in the tree), so one migration clears all three ignores. **Target:** [`fn-dsa`](https://crates.io/crates/fn-dsa) 0.4.0 — pure Rust, MSRV 1.82, `Unlicense` (already in the deny.toml allow-list), by Thomas Pornin, and named as the recommended replacement by the advisory itself. Split into `fn-dsa-{kgen,sign,vrfy,comm}`, so verify-only consumers (`veil-mailbox::capability`, ~50 LoC) can depend on the vrfy half alone. Not dependency-free, contrary to first impression: `fn-dsa-comm` pulls `cpufeatures` + `rand_core`, and `kgen`/`sign` add `zeroize` — all maintained, all already common in the tree. **Measured scope (2026-08-04, `git grep -c 'pqcrypto[_-]falcon'` / `'pqcrypto'` over tracked files):** direct `pqcrypto-falcon` references on **32 lines across 10 Rust files** plus 7 manifest lines in 6 `Cargo.toml`s; widening to any `pqcrypto` symbol gives **58 lines across the same 10 Rust files** (+11 manifest lines). Concentration is `veil-identity/src/signing_key.rs` (15), `veil-crypto/src/signature.rs` (14), `veil-identity/src/sovereign_flow.rs` (12); the tail is `veil-mailbox/src/capability.rs` (5), `veil-crypto/src/pow/score.rs` (3), then 1–2 each in `mlkem_fanout.rs` / `resolver.rs` / `verify.rs` / `sovereign.rs` / `veil-dht/src/kademlia.rs`. **Why it is parked and not merely unscheduled:** the blocker is the standard, not the crate. `fn-dsa`'s own README states that no FN-DSA draft has been published, that the 2026-07-22 release encodes a *"best guess"* at the coming FIPS 206 draft, and that **backward compatibility will explicitly NOT be maintained until a post-standard 1.0** — so migrating now buys an unmaintained-advisory cleanup at the cost of a wire format that may break again on the next upstream release. The break is concrete, not hypothetical: FN-DSA signatures are fixed-size (666 B at n=512, 1280 B at n=1024) where the round-3 Falcon encoding PQClean emits is variable-length under our `MAX_FALCON_SIG_BYTES = 768` / `MAX_FALCON1024_SIG_BYTES = 1462` caps; verifying-key sizes (897 / 1793 B) do match. FIPS 206 also changes hash-to-point and encoding relative to round-3 Falcon, so cross-version signature compatibility should be assumed absent. With no public release cut, that break is affordable — but confirm it against the `fn-dsa` docs again before starting, since the encoding is still a moving target. **Interim mitigation (already in place, operator decision 2026-08-04 — not urgent, no known exploit):** the verifier-side length caps above bound attacker-controlled input into the Falcon verifier at `crates/veil-crypto/src/signature.rs:391,400` (enforced at `:267`, `:313`, `:459`, `:494`). | `fn-dsa` publishes a post-standard **1.0** (i.e. after the final FIPS 206 text lands), **or** an actual vulnerability — not merely an `unmaintained` flag — is filed against any `pqcrypto-*` crate in the tree. |
