...
pub use quic::{
    PunchedQuicRole, QuicDatagramHandle, QuicDatagramSendError, REALTIME_DATAGRAM_BUFFER_BYTES,
...
mod tests {
    ...
    /// This must also run when `tls-boring` is enabled: that feature changes
    /// TCP TLS camouflage only, while peer QUIC deliberately stays on rustls
    /// so a handshake failure cannot trigger quinn-btls' process-wide panic.
    #[tokio::test(flavor = "current_thread")]
    async fn quic_registry_connect_accept_metadata_and_capabilities() {
        ...
        // A real peer has its own ephemeral self-signed transport cert. QUIC
        // must establish encryption with an independently generated context;
        // the signed Veil session handshake authenticates node identity next.
        let independent_client_ctx =
            Arc::new(TransportContext::for_debug().expect("independent client context"));
        let connection = registry
            .connect(&connect_uri, independent_client_ctx)
            .await
            .expect("client connects");
        assert_quic_connection_shape(connection.capabilities(), connection.peer_meta());
        let mut stream = connection.into_stream().expect("client stream");
