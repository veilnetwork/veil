...
//!
//! Without the RAII pattern а panic between dispatcher.register и
//! the explicit unregister call would leak the alias mapping, leading
//! к а route-table inconsistency on the next session for the same
...
//! * **Discoverability**: search для "AliasGuard" pre-extraction hit
//!   only one site (the `runner.rs` definition).  Post-extraction the
//!   module name surfaces in the file-listing pane.
//! * **Decoupling**: the constructor closure becomes а free function
//!   що takes the four needed inputs explicitly (`dispatcher`,
...
/// RAII guard що unregisters session aliases when dropped.
///
/// Constructed by [`register_session_aliases_with_drop_guard`].  Held
/// as а local variable in `run()` so it drops when the run loop exits
/// via any path (early return, normal exit, panic).
///
/// `#[allow(dead_code)]`: fields ара read solely by the `Drop` impl —
/// rustc otherwise warns "field never read".  Struct-level attribute
/// covers all three fields с а single anchor (per dead-code-anchors
/// policy).
#[allow(dead_code)]
...
/// Derive session aliases от `(session_id, node_id)` pairs, register
/// them with the dispatcher, и return а [`SessionAliasGuard`] что
/// auto-unregisters on drop.
///
/// Returns `None` for bootstrap / pre-handshake sessions where either
/// `session_id` или `peer_id` is the zero array — aliases rely on
/// session_id-bound derivation, so they're skipped когда unavailable.
///
/// Side-effect ordering: dispatcher registration MUST complete before
/// the [`SessionAliasGuard`] is constructed — а panic between would
...
// No standalone unit tests — `FrameDispatcher` requires а full
// session fixture к instantiate, so the zero-session-id и happy-path
// branches ара covered transitively via the existing session-runner
...
//   а SessionFsm directly без а runner, so it exercises the
//   "no aliases registered" path implicitly.
// * Healthy session с non-zero ids — covered by every other
//   `session::*` test что spawns а full SessionRunner via
