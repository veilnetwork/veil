...
fn open_beneath(root: &str, relative: &str) -> Result<VeilFsFile, String> {
    ...
    for (i, comp) in components.iter().enumerate() {
        let c = CString::new(*comp).map_err(|_| {
            // SAFETY: `dir` is open here.
            unsafe { libc::close(dir) };
            "NUL in a path component".to_owned()
        })?;
        let flags = if i == last {
        ...
        if next < 0 {
            let err = last_error();
            // ASK, do not guess. The first version of this decided from the
            // error TEXT, looking for "symbolic"/ELOOP — and a symlink to a
            // directory refused by `O_NOFOLLOW | O_DIRECTORY` reports ENOTDIR
            // on macOS, so the refusal was right and the sentence was wrong.
            // One `fstatat` on the failure path answers what the component
            // actually is, and it is asked relative to the same descriptor, so
            // it describes what the walk just refused rather than what the
            // name means now.
    ...
    // SAFETY: `dir` is the open file; `st` is writable.
    if unsafe { libc::fstat(dir, &mut st) } != 0 {
        let err = last_error();
        unsafe { libc::close(dir) };
