...
//! network with many node_ids and passively wait for honest nodes to add
//! them to routing tables.
//!
//! **Bucket pollution is the active variant**: when a sybil node receives
//! a FIND_NODE(target) query, instead of returning its real K-closest
//! contacts from its routing table it returns a CRAFTED list of FAKE
//! node_ids that look close to the queried target.  The querier's
//! iterative lookup then has to chase those fakes; in a naive
//! implementation each fake contact gets added to the local routing
//! table on receipt → routing-table eclipse without needing to run real
//! sybil nodes.
//!
...
//!
//! * **Layer 1 (strict-progress XOR filter)** — admit a contact only
//!   if its XOR distance to target is strictly less than the
//!   responder's distance.  Costs a sybil one real walk-closer step
//!   per crafted contact.  **Active.**
//! * **Layer 2 (per-responder cap)** — admit at most K=20 contacts
//!   per response.  Caps a single sybil's contribution to the shortlist
//!   regardless of how many fakes it returns.  **Active.**
//! * **Layer 3 (per-/16 AS-prefix cap)** — across all responders in a
//!   round, admit at most K/2 contacts pointing to the same /16
//!   transport prefix.  Caps a colluding-cluster-on-one-rented-subnet.
//!   **Planned** (referenced in `iterative.rs` doc, not yet wired).
//!
//! **End-to-end defence** beyond find_node_iterative: caller is expected
//! to PING / verify-liveness each returned contact before adding to the
//! routing table.  Fake contacts whose node_ids don't exist will fail
//! the ping and stay out of the routing table even if they made it
//! into the iterative result.
//!
//! This module ships a unit-level scenario that measures the
...
//! logic with a fraction of the infra cost, and does not require the
...
///
/// Behaves as `inner` for honest nodes.  For node_ids registered in
/// `adversary_replies`, ignores `inner` and returns the pre-baked crafted
/// contact list instead — independent of the actual `target` parameter
/// (the adversary doesn't know or care what the victim is looking up;
...
pub struct BucketPollutingPeerQuerier {
    ...
    /// Set of node_ids known to be sybil-originated (real adversary
...
impl BucketPollutingPeerQuerier {
    ...
    /// Also marks the adversary's own node_id and all crafted contact
    ...
    /// Test predicate: is `node_id` either a registered adversary OR
    /// a crafted-fake entry produced by one?  Used for the post-lookup
    ...
    fn find_node<'a>(
        ...
        Box::pin(async move {
            if let Some(reply) = crafted {
                ...
                // attacker would tune the fakes to share a leading prefix
                // with the queried target; the test fixture pre-builds
                // the crafted set with prefix-matched node_ids so this
...
mod tests {
    ...
    ///   FIND_NODE.  Each fake is a prefix-matched node_id (shares
    ///   target's top 28 bytes) so it scores ultra-close in XOR
    ///   distance and dominates the shortlist's K-closest cap.
    ...
    /// before promoting a contact to the routing table: each result
    /// contact is probed (`find_node` to it).  Live nodes respond with
    ...
    /// 3. After verification, the surviving contacts are all live
    ...
    async fn epic485_1_b_bucket_pollution_capped_by_iterative_walk_verification() {
        ...
        // random (we are not testing ID-grinding — orthogonal to
        // bucket-pollution).
        ...
        // Pick a random victim from the honest set + a random target.
        let victim_id = honest_ids[0];
        let mut target = [0u8; 32];
        OsRng.fill_bytes(&mut target);

        // Step 2 — build LocalPeerQuerier with each honest node's routing
        // table fully populated with all OTHER honest nodes (mesh).
        ...
        // Sybils have routing tables too — needed for them to look
        // "joined" to the victim.  But when queried, they will ignore
        // these and serve crafted fakes via the wrapper.
        ...
        // Step 3 — wrap querier with the bucket-pollution layer.  Each
        // sybil gets a unique crafted set of CRAFTED_PER_SYBIL fake
        // node_ids.  Sharing the prefix with the target makes the fakes
        ...
        for &sid in sybil_ids {
            ...
            for _ in 0..CRAFTED_PER_SYBIL {
                let mut fake = target;
                // Flip the LAST few bits — keeps top prefix shared with
                // target (looks close in XOR metric).
                let mut tail = [0u8; 4];
        ...
        // applies before promoting contacts to the routing table.  Each
        // returned contact is pinged via a follow-up `find_node`; if
        // the contact doesn't respond (empty result), it's dropped.
        //
        // This emulates production behaviour where `KademliaService` /
        // upper-layer DHT logic verifies each new contact's liveness
        // before adding it to the routing table.  Fakes (non-existent
        // node_ids) return empty here and stay out.
        let mut verified: Vec<Contact> = Vec::new();
        for contact in &raw_result {
            // Probe the contact with a random target — a live node would
            // answer with at least 1 contact from its routing table (it
            ...
            // they alive even if served fakes.  But a fake node_id
            // (one of the crafted contacts that doesn't correspond to
            // any actual node) returns empty from LocalPeerQuerier.
            if !probe.is_empty() {
        ...
        assert!(
            ...
            "raw iterative result must contain at least one sybil-fake \
             ...
             (no fakes were prefix-matched to target?)",
            raw_result.len(),
        );

        // Validation 2: verify-before-add MUST drop fake non-existent
        // node_ids.  Crafted fakes have node_ids that don't exist in
        // LocalPeerQuerier so their probe call returns empty → not
        // promoted to the verified set.
        let verified_fake_count = verified
            .iter()
            .filter(|c| {
                // Fake = sybil-flagged AND not a real adversary node
        ...
        assert_eq!(
            ...
            "verify-before-add MUST eliminate ALL crafted fakes: \
             {verified_fake_count} fakes survived verification.  Either a \
             fake node_id collided with a real one (~2^-32 probability) or \
             the polluter fixture leaked a fake to the mock querier",
        );

        // Validation 3: end-to-end posture summary (logs the actual
        // numbers for test diagnostics).
    ...
    /// Negative control: with **NO** adversaries, the iterative walk
    /// converges to a pure-honest result set (sybil fraction = 0).
    /// Catches a regression where `is_sybil` somehow false-positives
    /// honest contacts.
    #[tokio::test]
