...
impl KademliaService {
    ...
    ///
    /// Checks local store first, then walks the network sending FIND_VALUE frames.
    /// Returns `Some(value)` on first hit, `None` if unreachable.
    pub async fn find_value_iterative_network(
        ...
        {
            let mut inner = lock!(self.inner);
            if let Some(v) = inner.store.get(&key) {
                return Some(v.clone());
            }
        }
        let mut seeds: Vec<Contact> = {
        ...
        // Also seed from all peers with active sessions so nodes not yet in the
        // routing table (or not XOR-close to the key) are still reachable.
        // Use HashSet for O(1) dedup (was O(n²) with linear any).
        let mut seed_ids: std::collections::HashSet<[u8; 32]> =
            seeds.iter().map(|c| c.node_id).collect();
        for peer_id in outbox.peer_ids() {
        ...
        let params = super::iterative::IterativeParams::from(&self.dht_config);
        super::iterative::find_value_iterative(key, seeds, &querier, |_| None, &params).await
    }

    /// Propagate a value to the K closest network peers **without** storing locally.
    ///
    /// Sends STORE to:
    /// 1. All currently-connected peers (all `outbox` registrations) — ensures
    ///    direct neighbours always receive the value regardless of DHT routing.
    /// 2. The K closest peers discovered via iterative FIND_NODE — extends reach
    ///    beyond direct neighbours in larger networks.
    pub async fn publish_to_network(
        &self,
        key: [u8; 32],
        value: Vec<u8>,
        outbox: Arc<dyn FrameRouter>,
        ...
        // Collect targets: K-closest union all currently-connected peers.
        ...
        for target_id in targets {
            ...
            let payload = veil_proto::discovery::StorePayload::unsigned(key, value.clone());
            ...
            let _ = outbox.send_request(target_id, request_id, frame);
        }
    }

    /// Query the K-closest nodes **and all connected peers** for a key
    /// collecting every `Value` response instead of stopping at the first hit.
    ///
    /// Unlike [`find_value_iterative_network`], this method:
    /// does **not** short-circuit on local store (includes it in the result set)
    /// queries **all** candidate peers concurrently and collects all value
    /// responses, so the caller can pick the "best" one (e.g. highest PoW
    /// difficulty for name claims).
    ///
    /// Returns a deduplicated list of raw value bytes; may be empty if no peer
    /// holds the key.
    pub async fn find_all_values_network(
        &self,
        key: [u8; 32],
        outbox: Arc<dyn FrameRouter>,
    ) -> Vec<Vec<u8>> {
        // Collect targets: K-closest via iterative FIND_NODE + all connected peers
        // + a random sample from the full routing table (eclipse resistance).
        //
        // Eclipse resistance: if an attacker controls all of a victim's connected
        // peers they could suppress incumbent records during pre-claim lookup.
        // Querying a random subset of ALL routing-table contacts (not just the K
        // closest to the key) means the attacker must eclipse the routing table
        // entirely, not just occupy the key's neighbourhood.
        let closest = self
            .find_node_iterative_network(key, Arc::clone(&outbox))
            .await;
        ...
        // cap outbound parallelism to bound
        // amplification. The original implementation issued one
        // FIND_VALUE request per target with no upper bound — at peak
        // the routing table contributes ALPHA*3 entries, the K-closest
        // contributes K, plus every connected peer. An attacker with
        // plenty of accessible peers could then arm one
        // `find_all_values_network` to amplify into hundreds of
        ...
        for result in results {
            if let super::iterative::FindValueResult::Value(v) = result {
                values.push(v);
            }
        }
        values
