...
pub enum AdminCommand {
    ...
    ListBans,
    /// Publish а P-Net DHT-replicated ban. Requires `[network].mode =
    /// "private"`, а local membership cert with `admin: true`, и а
    /// loaded identity signing key. Signs а `BanEntry`, fans it out к
    /// the network через DHT replication, и applies к the local
    ...
    /// Builds а `DeliveryMsg::RelayPath` frame from an explicit path of
    /// hex node_ids и hands it off к the first hop's session.  Каждый
    /// intermediate forwards к the next entry в `path`; the terminal
    /// hop decodes the inner `AppSendPayload` и delivers к the local
    /// app registry.  Works в topologies где DHT routing fails
    ...
    MobileStatus,
    /// **Push а new config к the running daemon без going через
    /// the filesystem.**  Distinct от [`Self::Reload`] (which
    /// re-reads `self.config_path`):  here the caller supplies the
    /// raw TOML bytes inline, и optionally requests persistence к
    /// disk.  Intended use cases:
    ///
    /// * **Messenger build** — app stores config in а secure
    ///   storage backend (Keychain / EncryptedSharedPreferences / а
    ///   future TPM-sealed store), not on the regular filesystem,
    ///   и pushes the config к the embedded veil daemon at
    ///   startup time с `persist: false`.
    /// * **Server admin** — programmatically applying configs
    ///   generated by orchestration (Terraform output piped к
    ...
    /// * **Deferred-init startup** — combined с the `--defer-init`
    ///   CLI flag, the daemon boots без an identity, binds only the
    ///   admin socket, и this command provides the actual config
    ///   that promotes the daemon к full operation.
    ///
    /// Validation is performed before applying — а malformed или
    /// invalid config is rejected с `AdminResult::Error` и leaves
    /// the running daemon unchanged.  Network state is fully
    /// re-initialised on apply (same path as [`Self::Reload`]) —
    /// sessions, transports, и services restart с the new config.
    ApplyConfig {
        /// Raw TOML config content. Pre-parse so invalid TOML
        /// gets rejected с а structured error before touching daemon
        /// state.
        toml_content: String,
        /// Whether к persist the new config к the daemon's
        /// configured `config_path` after а successful apply.
        /// `false` keeps the change in-memory only (the daemon
        /// returns к defaults on next restart).  Default `false`
...
/// one round-trip без grepping session-runner logs.
///
/// All fields read from в-memory state OR /sys (Linux battery) —
/// no network I/O. Read at the moment the admin request is
/// served, so flips через `SetMobileBackgroundMode` are visible
/// immediately.
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
pub struct AdminMobileStatus {
    ...
    /// Composes multiplicatively с battery scaling.
    pub background_keepalive_factor: u32,
    /// Current battery reading. `100` = AC / unknown / non-Linux
    /// sentinel — never throttled on this signal по design.
    ...
    /// `low_battery_multiplier` clamped к
...
/// Start the daemon в **deferred-init mode** — boot с а stub config
/// (ephemeral Ed25519 identity, empty peers / listens) и await а
/// runtime `admin apply-config` к provide the real config.
///
/// Concretely:
/// 1. Generate а fresh per-run Ed25519 keypair и mine its PoW nonce.
/// 2. Build а minimal Config (just `[identity]` + defaults).
/// 3. Create а fresh temp directory (`$TMPDIR/veil-deferred-<pid>-<ts>`)
///    so daemon-owned working files (`mlkem.key`, future `identity_document.bin`)
///    don't collide с existing instances and are reaped по process exit
///    on modern OSes.
/// 4. Write the stub config к `<tmpdir>/node.toml` (atomic-write — the
///    temp dir is только writable by the daemon's UID anyway).
/// 5. Hand off к `run_foreground_with_shutdown` exactly as if а normal
...
/// network под the ephemeral keypair — but без any peers configured и
/// без any listen ports, so no traffic exists to be ascribed к it.
pub async fn run_foreground_deferred() -> Result<()> {
    // Build stub config (CPU-heavy — runs PoW search; tolerated because
    // deferred-init is а one-time startup cost).
    ...
    // Hand off к the normal foreground loop.  `--foreground` is implied
    // — а background-spawned daemon would inherit the temp dir but lose
    ...
    F: std::future::Future<Output = ()> + Send + 'static,
{
    ...
    // semaphore caps "how many concurrent admin connections а single
    // authorised UID can hold open" so а bug or mis-tooling cannot
    // exhaust resources by spawning hundreds of admin clients. When
    // the cap is hit, the accept loop logs `admin.accept_refused` и
    // drops the connection без spawning а handshake task (saves а
    // task slot relative к the pre-cap behaviour).
        ...
        loop {
            tokio::select! {
                ...
                accepted = listener.accept_raw() => {
                    ...
                    // immediately после kernel `accept(2)`; the 32-byte
                    // token handshake (which can stall up к 3 s on а
                    // malicious connect-and-hang client) runs inside the
                    // spawned task ниже. Pre-fix, одно stalled handshake
                    // blocked the entire admin loop для 3 s × N attempts.
                    let (pending, peer_info) = match accepted {
                        Ok(pair) => pair,
                        Err(e) => {
                            // Per-connection accept errors must not kill
                            // the listener — log at info level и keep
                    ...
                    // the handshake task — saves а task spawn for the
                    ...
                    // на а full semaphore returns Err immediately (no blocking
                    // на the accept loop). Drop the connection и log.
                    ...
                    tokio::spawn(async move {
                        ...
                        // immediately и slot frees.
                        let _permit = permit;
                        // complete the token-handshake step
                        // here (off the accept loop). Failures (timeout
                        // mismatch) are logged at info level и drop the
                        // connection silently — а probe или misconfigured
...
async fn handle_admin_connection(
    ...
    // К MAX_ADMIN_REQUEST_BYTES to prevent а connected client от
    // exhausting daemon memory by streaming arbitrary bytes без
    // newline.  `tokio::io::AsyncBufReadExt::read_line` reads до
    // either а newline OR EOF; without а cap, а misbehaving (or
    // attacker-controlled) Unix-socket / TCP-loopback client could
    // grow `line: String` к gigabytes before failing.  The 64 KiB
    // cap is generous — the largest legitimate admin command
    // (RelaySend с а 16-hop path в hex) clocks ~2 KiB.
    ...
    if line.len() > MAX_ADMIN_REQUEST_BYTES {
        ...
        // adversary noise; ban-list integration belongs at а higher
        // layer if we want к escalate.
        return Ok(());
    }
...
async fn execute_admin_command(
    ...
    let result = match command {
        ...
        AdminCommand::ApplyConfig {
            ...
            // Delegate к the via_arc variant so the long-running
            // stop-tasks phase doesn't hold the outer Arc<Mutex<...>>
            // lock — concurrent admin commands (e.g., а Show issued
        ...
        AdminCommand::RelaySend {
            ...
            // layer к detect а loop и drop the frame mid-flight,
            // leaving the operator unable к diagnose the silent
            // failure.  Surfacing as а clean error at the admin
...
pub fn audit_event_for(
    ...
    let (kind, args) = match command {
        ...
        AdminCommand::ApplyConfig {
            ...
            "apply_config",
            // Don't log raw config content — может содержать identity
            // keys / passphrases / token secrets.  Log just the byte
            // length и persist-flag so audit trail can correlate с
            // expected ops batches без leaking secrets.
...
pub fn spawn_restart_child(config_path: &Path) -> Result<()> {
    ...
    // scrub env to а PATH/HOME/locale allow-list
    // before spawn. Inherited `LD_PRELOAD`, `LD_LIBRARY_PATH`
    // `RUST_LOG`, `VEIL_*` etc. would все survive on а raw `Command`
    // и could redirect the long-lived daemon's behaviour. See helper
    // doc для full allow-list / rationale.
...
mod tests {
    ...
    /// **ApplyConfig (Phase 1)**: push а new config к the running daemon
    /// via IPC bytes (no filesystem intermediary).  Verify:
    ///  1. Successful apply returns AdminResult::Ack.
    ///  2. The new listen-entry is reflected в the subsequent Listens query.
    ...
    async fn apply_config_inmemory_updates_runtime_without_disk_write() {
        ...
        // Build а new config from the loaded one + an extra listen entry.
        let mut updated = veil_cfg::load_config(&path).unwrap();
        updated.listen.push(ListenConfig {
            id: ListenId::new(7),
            transport: "tcp://127.0.0.1:0".to_owned(),
        ...
        // Round-trip через veil_cfg::save → read so we get а TOML-shaped string.
        // Use а throwaway temp path so the original `path` stays untouched
        // (we ара testing the in-memory apply, not file persistence).
    ...
    async fn apply_config_rejects_invalid_toml() {
        ...
        // Garbage that's neither valid TOML nor а valid Config.
        let response = send_request(
            &socket,
            AdminCommand::ApplyConfig {
    ...
    /// **--defer-init (Phase 2)**: stub-config builder produces а valid
    /// Config с an ephemeral identity that passes its own validation.
    /// Without this property, deferred-init startup would refuse к boot.
    ///
    /// Cheap unit test (no daemon spin-up) — just verifies the builder
    /// invariants.  Daemon-level smoke test would require а full
    /// `NodeRuntime::start` call which is heavy для CI и duplicates
    ...
    fn defer_init_stub_config_is_valid() {
        ...
        // daemon comes up but doesn't try к touch the network until
        // а real ApplyConfig arrives.
    ...
    /// **ApplyConfig (Phase 1)** с `persist: true` writes the new
    /// config к `self.config_path` (atomic-write) и а subsequent
    ...
    // directional dedup (see comment в
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send,
    {
        ...
        perform_ovl1_handshake(
            ...
            false,
            None,
            None, // P-Net: no network gate в admin test fixture
