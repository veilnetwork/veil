//! `PnetStatusProvider` implementation що exposes the daemon's
//! per-peer verified-cert cache к IPC consumers (ogate / oproxy).
//!
//! Lookup is а brief read-lock against the `verified_peer_certs`
//! HashMap shared с the rest of the runtime.  When P-Net is not
//! enabled (gate=None), the cache stays empty и all queries reply
//! `admitted=false / has_cert=false`.
//!
//! `admitted` is derived от `live_sessions`: even если а cert is
//! cached, the peer might have disconnected since; surfacing а
...
/// so а direct query is O(N) — acceptable for testnet но not для
...
impl DaemonPnetStatus {
    ...
    fn is_admitted(&self, peer_node_id: &[u8; 32]) -> bool {
        ...
        // (testnet: ≤ 100, production sessions plane uses up к 65K
        // per Epic 302 sizing).  Acceptable for а per-stream IPC query
