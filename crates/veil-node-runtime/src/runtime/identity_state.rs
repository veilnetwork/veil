...
//! extracted into а dedicated [`Arc<IdentityState>`].
//!
//! ## Why а dedicated struct
//!
//! Pre-PR5, `NodeRuntime` held eight identity-domain fields directly:
//! the daemon's own handshake / sovereign identity, и five peer-side
...
//! и groups crypto-identity state together for navigation.
//!
//! ## Reload semantics
//!
//! `local_identity` и `sovereign_identity` are immutable per-process —
...
//! без replacing the Arc, so downstream Arc-clone holders observe new
//! state automatically.
//!
//! ## What's NOT in here
//!
//! `NodeServices` и `SessionRuntimeContext` continue к carry their own
//! direct identity-field clones (built by Arc-clone от NodeRuntime's
...
//! count meaningfully relative к the migration cost (~74 callsites
//! against the 3 contexts combined); this PR limits scope к NodeRuntime
...
pub struct IdentityState {
    ...
    /// optional sovereign-identity handle loaded от disk.
    /// `None` on legacy nodes (pre-462) — they fall back к the
    ...
    /// Cache of `(peer_node_id) → (algo, raw_pubkey_bytes)` для all
    /// peers we've successfully completed an OVL1 handshake with.
    /// Used by the dispatcher's relay-send path к verify cryptographic
    ...
    /// `reload_with` so session-resumption fast paths (которые bypass
    ...
    /// Maps `peer_id → roles_supported` bitmask от the handshake (
    /// Cross-checked against advertised capabilities preventing
    /// Gateway-role spoofing.
    pub peer_roles: Arc<Mutex<PeerLruCache<u8>>>,

    /// Local ML-KEM-768 encapsulation key — sent к remotes during
    /// handshake so peers can encrypt payloads for this node.
    pub mlkem_ek: Arc<[u8; EK_BYTES]>,

    /// Peer ML-KEM-768 key cache — populated после each handshake.
    /// Shared с `FrameDispatcher` so the relay-send path can encrypt E2E.
    pub peer_mlkem_keys: Arc<RwLock<PeerMlKemCache>>,

    /// per-session ephemeral ML-KEM DK seeds shared с
    /// `CryptoContext`. Maps `peer_id → dk_seed`; shared с
    /// `FrameDispatcher` для E2E decryption.
    ///
    /// Этап 6 slice 6h — value type wrapped в `SensitiveBytesN<64>` so
    /// per-session DK seeds ара mlocked while the session is open.
