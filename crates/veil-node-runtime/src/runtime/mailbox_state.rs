...
//! extracted into a dedicated [`Arc<MailboxState>`].
//!
...
//! sprinkled with DHT / session / anonymity state. Both are
//! Optional-Arc handles populated at startup based on operator config
//! and never reassigned at runtime — so a dedicated struct doesn't add
//! mutex bookkeeping, but it does:
//!
//! 1. Gives slice-3 follow-ups a natural home (per-sender quota
//!    counters, capability-token enforcement state, push dispatcher
//!    references, eventually mailbox-replication coordinator).
//! 2. Keeps `NodeRuntime` reads against a typed domain bundle —
//!    `self.mailbox_state.mailbox` / `.outbox` instead of two siblings
//!    that need to be mentally grouped.
//! 3. Mirrors the `AnonymityState` pattern shipped in PR1.
//!
//! ## Migration surface
//!
//! Each callsite reading `self.mailbox` / `self.outbox` now reads
//! `self.mailbox_state.mailbox` / `.outbox`. Builder collapses the
//! two `Option<Arc<...>>` builder vars into a single `MailboxState::new`
...
/// when `Outbox::open` succeeded — peer-sync is a universal feature
