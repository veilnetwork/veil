...
//!
//! ## Why а dedicated struct (and why not `Arc<...>`)
//!
//! Pre-stage-B, `NodeRuntime` carried four PEX-related fields side-by-
//! side с unrelated session/handoff/mailbox state:
//!
//! 1. `pex_state: Arc<Mutex<PexState>>` — shared с the initiator task.
...
//! that pattern: `tokio::mpsc::Receiver` is not `Clone`, и the
//! receivers must be `.take()`-d via `&mut self` at task-spawn time.
//! Wrapping в `Arc<Mutex<_>>` just to satisfy take-once semantics
//! would add а lock that nobody contends on. Instead, `PexRuntime`
//! is а plain owned struct embedded directly в `NodeRuntime`;
//! NodeServices never carries PEX state, so there is no propagation
//! surface к worry about.
//!
//! Net effect: 4 sibling fields → 1 typed bundle. Same migration
//! pattern as the Arc-shared bundles но with an explicit comment
...
pub struct PexRuntime {
    /// PEX shared state — Arc-wrapped мьютекс so multiple tasks can
    /// read while the initiator task mutates.
    pub state: Arc<Mutex<veil_pex::PexState>>,

    /// PEX event receiver. Consumed once by `spawn_pex_initiator`; на
    /// subsequent reloads this is `None` и the initiator does not
    ...
    /// PEX connect sender. Cloned into the initiator task на spawn.
    pub connect_tx: Option<veil_pex::PexConnectTx>,

    /// PEX connect receiver, consumed once by the PEX connector task
    /// that reads discovered peers и initiates outbound connections.
