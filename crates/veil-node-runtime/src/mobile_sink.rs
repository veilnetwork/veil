...
pub struct MobileEventForwarder {
    ...
    /// of the now-stale TCP) and `has_session` pre-check returns false
    ...
    /// "power-saving" UI without polling `GetMobileStatus`.
...
impl MobileEventSink for MobileEventForwarder {
    ...
    fn network_changed(&self, payload: NetworkChangedPayload) {
        ...
        // loop to retry IMMEDIATELY on the new local interface.
        ...
        // ~30-90 s (TCP keepalive timeout + 30 s pre-check sleep) to
