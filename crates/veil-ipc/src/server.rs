...
impl IpcServer {
    ...
    /// Attach the shared ratchet conversations, so one-to-one sends can run
    /// key agreement instead of a bare encapsulation to a published key.
    #[must_use]
    pub fn with_ratchet(mut self, ratchet: veil_e2e::RatchetRuntime) -> Self {
