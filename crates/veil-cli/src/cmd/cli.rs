...
pub enum IdentityCommand {
    ...
    /// key, and saves the updated document. The previous subkey is not
    /// revoked (there is no revocation) — it simply ages out via its short
    /// `valid_until_unix` window. Routine key hygiene that does not change
    /// your identity's name.
