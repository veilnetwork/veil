...
pub struct Args {
    ...
    /// Optional authenticated loopback service that selects a SOCKS5 address
    /// for each accepted flow. Programmatic API only; CLI behavior stays
    /// identical to upstream tun2proxy 0.8.2.
    #[arg(skip)]
    #[serde(skip)]
    pub proxy_selector: Option<ProxySelectorConfig>,
...
impl Args {
    ...
    pub fn proxy_selector(&mut self, selector: ProxySelectorConfig) -> &mut Self {
